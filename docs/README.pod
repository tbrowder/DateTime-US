...
Module B<DateTime::US> provides a class and functions used to help Raku programs needing US time zone and DST information.
The user creates a DateTime::US object by providing a US time zone abbreviation.  The list of known data may be
...
=head2 U<Class DateTime::US methods>

=head3 to-localtime(DateTime :$utc! --> DateTime) {...}

Given a time in UTC, convert to local time and adjust for DST.

=head3 to-utc(DateTime :$localtime! --> DateTime) {...}

Given a local time, convert to UTC with adjustment for DST.

=head2 U<DateTime::US subroutines>

=head3 begin-dst($year --> DateTime) {...}

Return the time when DST begins.

=head3 end-dst($year --> DateTime) {..}

Return the time when DST ends.

=head3 is-dst(DateTime :$localtime! --> Bool) {...}

Return True if the local time is DST, otherwise return False.

=head3 show-us-data(--> Str) {...}
